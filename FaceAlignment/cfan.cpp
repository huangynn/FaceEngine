...
void CCFAN::FacialPointLocate(const unsigned char *gray_im, int im_width, int im_height, seeta::FaceInfo face_loc, float *facial_loc)
{
    ...
    int extend_lx = std::fmax(floor(left_x - extend_factor*bbox_w), 0);
    int extend_rx = std::fmin(floor(right_x + extend_factor*bbox_w), im_width - 1);
    int extend_ly = std::fmax(floor(left_y - (extend_factor - extend_revised_y)*bbox_h), 0);
    int extend_ry = std::fmin(floor(right_y + (extend_factor + extend_revised_y)*bbox_h), im_height - 1);
...
void CCFAN::GetSubImg(const unsigned char *gray_im, int im_width, int im_height, float point_x, float point_y, int patch_size, BYTE *sub_img)
{
    ...
    int lx = std::fabs(patch_left - ((center_x + 1) - patch_size / 2));
    int rx = patch_size - std::fabs(patch_right - ((center_x + 1) + patch_size / 2 - 1)) - 1;
    int ty = std::fabs(patch_top - ((center_y + 1) - patch_size / 2));
    int by = patch_size - std::fabs(patch_bottom - ((center_y + 1) + patch_size / 2 - 1)) - 1;
